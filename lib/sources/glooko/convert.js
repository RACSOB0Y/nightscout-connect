  ...
  */
  ...
  if (foods) {
    foods.forEach(function(element) {
      ...
      treatment.enteredBy = `${element.pumpName} via Glooko`;

      treatment.carbs = element.carbs;
      treatment.notes = element.pumpName;
...
function generate_nightscout_entries(batch, timestampDelta) {
  ...
  console.log("TEST LOG batch", batch);
  console.log("TEST LOG readings", readings);
  var entries = [];
  ...
  if (Array.isArray(readings)) {
    readings.forEach(function(element) {
      ...
      var r_date = new Date(element.timestamp);
      var r_s_date = new Date(r_date.getTime() + timestampDelta);

      entry.type = 'sgv';
      entry.dateString = new Date(r_s_date).toISOString();
      entry.date = r_s_date.getTime();
      entry.sgv = element.value/100;
      entry.units = "mg/dl"
      entry.direction = "Flat"; // TODO: Calculate direction
      entry.noise = 0;
      entry.filtered = element.value;
      entry.unfiltered = 0;
      entry.rssi = 0;
    ...
    console.log('Readings is not an array:', typeof readings);
  }
