...
function glookoSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      var two_days_ago = new Date( ).getTime( ) - (7 * 24 * 60 * 60 * 1000); // 1 week
      ...
      const ytdDate = new Date(myDate);
      ytdDate.setDate(myDate.getDate() - 7); // 1 week ago
      const ytdDateString = ytdDate.getFullYear() + '-'
         + ('0' + (ytdDate.getMonth()+1)).slice(-2) + '-'
         + ('0' + ytdDate.getDate()).slice(-2);
      
      console.log('SESSION USER', session.user);
      let v3APIURL = Defaults.v3API.replace('_PATIENT_',session.user.userLogin.glookoCode).replace('_STARTDATE_', ytdDateString + "T00:00:00.000Z").replace('_ENDDATE_', dateString + 'T23:59:59.999Z');
