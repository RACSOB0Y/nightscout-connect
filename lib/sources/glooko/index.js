...
function glookoSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      const lastGlucoseDateString = last_glucose_at.getFullYear() + '-'
         + ('0' + (last_glucose_at.getMonth()+1)).slice(-2) + '-'
         + ('0' + last_glucose_at.getDate()).slice(-2);
      
      console.log('SESSION USER', session.user);
      let v3APIURL = Defaults.v3API.replace('_PATIENT_',session.user.userLogin.glookoCode).replace('_STARTDATE_', lastGlucoseDateString + "T00:00:00.000Z").replace('_ENDDATE_', dateString + 'T23:59:59.999Z');
