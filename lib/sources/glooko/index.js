...
, v3API: '/api/v3/graph/data?patient=_PATIENT_&startDate=_STARTDATE_&endDate=_ENDDATE_&series[]=cgmHigh&series[]=cgmLow&series[]=cgmNormal&locale=en-GB'
//, v3API: '/api/v3/graph/data?patient=_PATIENT_&startDate=_STARTDATE_&endDate=_ENDDATE_&series[]=basalBarAutomated&series[]=basalBarAutomatedSuspend&series[]=carbNonManual&series[]=cgmHigh&series[]=cgmLow&series[]=cgmNormal&series[]=deliveredBolus&series[]=pumpAdvisoryAlert&series[]=pumpAlarm&series[]=pumpOp5AutomaticMode&series[]=pumpOp5HypoprotectMode&series[]=pumpOp5LimitedMode&series[]=pumpOp5ManualMode&series[]=reservoirChange&series[]=setSiteChange&locale=en-GB'
...
function glookoSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      var two_weeks_ago = new Date( ).getTime( ) - (14 * 24 * 60 * 60 * 1000); // 2 weeks ago
      var last_mills = Math.max(two_weeks_ago, (last_known && last_known.entries) ? last_known.entries.getTime( ) : two_weeks_ago);
      ...
      function fetcher (endpoint, params_to_use) {
        ...
        console.log('GLOOKO FETCHER LOADING', endpoint, "PARAMS", params_to_use);
        return http.get(endpoint, { headers, params: params_to_use })
          .then(response => {
            console.log('GLOOKO FETCHER RESPONSE RECEIVED');
            return response.data;
          })
          .catch(error => {
            console.error('GLOOKO FETCHER ERROR', error);
            throw error;
          });
      }
      ...
      const fortNightDate = new Date(myDate);
      fortNightDate.setDate(myDate.getDate() - 14); // 2 Weeks ago
      const fortNightDateString = fortNightDate.getFullYear() + '-'
         + ('0' + (fortNightDate.getMonth()+1)).slice(-2) + '-'
         + ('0' + fortNightDate.getDate()).slice(-2);
      
      console.log('SESSION USER', session.user);
      let v3APIURL = Defaults.v3API.replace('_PATIENT_',session.user.userLogin.glookoCode).replace('_STARTDATE_', fortNightDateString + "T00:00:00.000Z").replace('_ENDDATE_', dateString + 'T23:59:59.999Z');
      ...
      function constructUrl(endpoint) {
        ...
        const myDate = new Date();
        const startDate = new Date(two_weeks_ago); // myDate.getTime() - 6 * 60 * 60 * 1000);
      ...
      return Promise.all([
        ...
        fetcher(constructUrl(Defaults.LatestPumpBasals), params),
        fetcher(constructUrl(Defaults.LatestPumpBolus),params),
        fetcher(v3APIURL,{}),
        ...
        //fetcher(constructUrl(Defaults.PumpSettings))
         ...
         var some = {
            ...
            readings: results[2].series.cgmLow.concat(results[2].series.cgmNormal, results[2].series.cgmHigh),
            cgm: results[2].series.cgmDeviceDataBrand
    ...
    transformData (batch) {
      // TODO
      console.log('GLOOKO passing batch for transforming -  keys: ', Object.keys(batch));
      ...
      console.log("E.g. entries:", entries.slice(0, 5));
  ...
  function generate_driver (builder) {
    ...
    builder.register_loop('Glooko', {
      ...
      // expect new data 6 hours after last success
      expected_data_interval_ms: 6 * 60 * 60 * 1000,
      backoff: {
        // wait 2.5 minutes * 2^attempt
        interval_ms: 2.5 * 60 * 1000
      },
