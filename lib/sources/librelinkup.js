...
function linkUpSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      var two_weeks_ago = new Date( ).getTime( ) - (14 * 24 * 60 * 60 * 1000);
    ...
    transformGlucose (payload, last_known) {
      ...
      var backfill_from = (last_known && last_known.entries) ? last_known.entries : two_weeks_ago;
      ...
      // Convert graphData (no TrendArrow) and the current reading (has TrendArrow) separately.
      // Deduplicate by timestamp, preferring the current reading so direction is preserved.
      var graphEntries = batch.graphData.map(to_ns_sgv).filter(is_newer);
      var currentSgv = batch.connection && batch.connection.glucoseMeasurement
        ? to_ns_sgv(batch.connection.glucoseMeasurement)
        : null;
      var entries = currentSgv
        ? graphEntries.filter(e => e.date !== currentSgv.date).concat(is_newer(currentSgv) ? [currentSgv] : [])
        : graphEntries;
      var treatments = [ ];
      var devicestatus = [ ];
      var profiles = [ ];
